...
export default function AdminDashboard() {
  ...
  const fetchPending = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/members?status=pending`
      );
  ...
  return (
    <div className="relative max-w-6xl mx-auto px-4 py-8">
      <button
        onClick={() => {
          localStorage.removeItem("trc_admin_logged_in");
          window.location.href = "/admin-login";
        }}
        className="absolute top-4 right-4 text-sm bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700"
      >
        Logout
      </button>
